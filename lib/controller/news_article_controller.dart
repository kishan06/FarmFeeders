...
class NewsArticleController extends GetxController {
  ...
  changeBookmark(int index,{bool isBookmarkedList = false}){
    if (isBookmarkedList) {
      _bookmarkedNewsArticle!.data[index].bookmarked = !_bookmarkedNewsArticle!.data[index].bookmarked;
      update();
    } else {
  ...
  GetBookmarkedList() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // print(response.data);
        // _newsArticlesData = NewsArticlesModel.fromJson(response.data);
        _bookmarkedNewsArticle = BookmarkedNewsListModel.fromJson(response.data);
  ...
  bookmarkApi({required int index, required String id, bool isBookmarkList = false}) async {
    try {
      print("$bearerToken");
      var headers = {
        'Authorization': bearerToken
